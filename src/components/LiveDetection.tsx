...
import { Camera, CameraOff, Loader2, Download, Circle, FileVideo } from "lucide-react";
...
const LiveDetection: React.FC<LiveDetectionProps> = ({ modelPath }) => {
  ...
  const handleExportMp4 = async () => {
    if (recordedBlob) {
      await exportVideo(recordedBlob, `newtoplot-live-${Date.now()}.mp4`);
    }
  };
              ...
              {isLoading ? (
                <div className="flex flex-col items-center gap-6">
                  ...
                  <div className="flex flex-col items-center gap-1">
                    <p className="text-[var(--fg)] font-display font-bold text-lg tracking-tight">Initializing AI Engine</p>
                    <p className="text-gray-400 text-[10px] font-black uppercase tracking-[0.2em]">Loading Neural Weights</p>
                  </div>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-6">
                  <div className="p-8 rounded-[32px] bg-[var(--glass)] border border-[var(--glass-border)] shadow-inner">
                    <CameraOff className="w-14 h-14 text-gray-400" />
                  </div>
                  <div className="flex flex-col items-center gap-1 text-center">
                    <p className="text-gray-400 font-display font-bold text-lg tracking-tight">System Standby</p>
                    <p className="text-gray-500 text-[10px] font-black uppercase tracking-[0.2em]">Awaiting Stream Activation</p>
                  </div>
                </div>
              )}
            </motion.div>
